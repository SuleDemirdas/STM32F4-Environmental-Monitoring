/**
 * @file main.c
 * @brief Main program body for the STM32F4 multi-sensor data acquisition system.
 *
 * This application reads humidity and temperature data from an AHT20 sensor,
 * illuminance from a BH1750 sensor, and atmospheric pressure from a BMP180
 * sensor over I2C. Raw readings are filtered with a sliding-window median filter
 * and stored in separate circular (ring) buffers. Every 30 seconds the system
 * computes min, max, median and standard deviation for each sensor and transmits
 * the results over USART2 at 115200 baud in the following format:
 *
 * @code
 ...
 * @endcode
 *
 * A 1-second tick is generated by TIM3 configured on the APB1 bus
 * (prescaler = 12499, period = 999, resulting in a 1 Hz interrupt at 12.5 MHz).
 *
...
/**
 * @brief TIM3 period-elapsed callback — fires every 1 second.
 *
 * Sets @ref read_sensor_flag to notify the main loop that a new sample should
 * be acquired, and toggles the green LED as a heartbeat indicator.
 *
 * @param[in] htim Pointer to the TIM_HandleTypeDef that triggered the callback.
 */
